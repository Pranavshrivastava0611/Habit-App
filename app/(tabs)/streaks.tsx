...
import { Habit, completedHabit } from '@/types/database.type';
import { useAuth } from '@/lib/authProvider';
import { DATABASE_ID, COLLECTION_ID, HABIT_COLLECTION_ID } from '@/lib/appwrite';
import { databases } from '@/lib/appwrite';
import { Query } from 'react-native-appwrite';
import { client } from '@/lib/appwrite';
import { RealtimeResponse } from '@/lib/appwrite';

export default function LoginScreen() {
  const [Habits, setHabits] = useState<Habit[]>([]);
  const [completedTask, setCompletedTask] = useState<completedHabit[]>([]);
  const { user } = useAuth();
  ...
  const fetchCompletion = async () => {
    ...
    try {
      ...
      setCompletedTask(response.documents as completedHabit[]);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const streakData = (habitId: string): StreakData => {
    const habitArr = completedTask
  ...
  const habitStreaks = Habits.map((habit) => {
    const { streak, bestStreak, total } = streakData(habit.$id);
        ...
        <View style={styles.rankingContainer}>
          ...
          {rankedHabits.slice(0, 3).map((item, key) => (
            <View key={key} style={styles.rankingRow}>
              ...
              <Text style={styles.rankingHabit}>{item.habit.title ?? 'Untitled Habit'}</Text>
